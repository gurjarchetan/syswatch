...
fn gradient_bar(pct: f64, width: usize) -> Vec<Span<'static>> {
    ...
    for i in 0..width {
        ...
        let color = if i < filled { theme::pct_color(pos_pct.max(pct * 0.3)) }
                    else { theme::C_BORDER };
...
pub fn render(f: &mut Frame, area: Rect, app: &AppState) {
    ...
    ram_spans.push(Span::styled(
        format!(" {:5.1}%", used_pct),
        Style::default().fg(theme::pct_color(used_pct)).add_modifier(Modifier::BOLD),
    ));
    ...
    lines.push(Line::from(vec![
        ...
        Span::styled(MemStats::fmt_kb(mem.used_kb), Style::default().fg(theme::C_WHITE)),
        Span::styled(" / ", theme::dim_style()),
        Span::styled(MemStats::fmt_kb(mem.total_kb), theme::dim_style()),
        Span::styled("  free ", theme::dim_style()),
        Span::styled(MemStats::fmt_kb(mem.free_kb), Style::default().fg(theme::C_GREEN)),
    ]));
    ...
    swap_spans.push(Span::styled(
        format!(" {:5.1}%", swap_pct),
        Style::default().fg(theme::pct_color(swap_pct)).add_modifier(Modifier::BOLD),
    ));
    lines.push(Line::from(swap_spans));
    lines.push(Line::from(vec![
        Span::styled("     ", theme::dim_style()),
        Span::styled(MemStats::fmt_kb(mem.swap_used_kb), Style::default().fg(theme::C_WHITE)),
        Span::styled(" / ", theme::dim_style()),
    ...
    if graph_h >= 2 && !state.mem_history.is_empty() {
        ...
        for (i, row_str) in rows.iter().enumerate() {
            let lbl = if i == 0 { "100%".to_string() }
                      else if i == graph_h / 2 { " 50%".to_string() }
                      else if i + 1 == graph_h  { "  0%".to_string() }
                      else { "    ".to_string() };
            lines.push(Line::from(vec![
