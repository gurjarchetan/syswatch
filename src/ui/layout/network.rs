...
pub fn render(f: &mut Frame, area: Rect, app: &AppState) {
    ...
    lines.push(Line::from(vec![
        ...
        Span::styled(NetStats::fmt_speed(net.rx_bps),
            Style::default().fg(theme::C_WHITE).add_modifier(Modifier::BOLD)),
        Span::styled(format!("  total {} ", NetStats::fmt_bytes(net.total_rx)), theme::dim_style()),
        Span::styled("  ▲ TX ", Style::default().fg(theme::C_MAGENTA).add_modifier(Modifier::BOLD)),
        Span::styled(NetStats::fmt_speed(net.tx_bps),
            Style::default().fg(theme::C_WHITE).add_modifier(Modifier::BOLD)),
        Span::styled(format!("  total {}", NetStats::fmt_bytes(net.total_tx)), theme::dim_style()),
    ]));
    ...
    lines.push(Line::from(vec![
        ...
        Span::styled(format!("{:>12}  ", "▼ RX/s"),
            Style::default().fg(theme::C_TEAL).add_modifier(Modifier::BOLD)),
        Span::styled(format!("{:>12}", "▲ TX/s"),
            Style::default().fg(theme::C_MAGENTA).add_modifier(Modifier::BOLD)),
    ]));

    for iface in &net.interfaces {
        lines.push(Line::from(vec![
            Span::styled(format!("  {:<14}", crate::ui::truncate(&iface.name, 12)), theme::dim_style()),
            Span::styled(format!("{:>12}  ", NetStats::fmt_speed(iface.rx_bps)),
                Style::default().fg(theme::C_TEAL)),
            Span::styled(format!("{:>12}", NetStats::fmt_speed(iface.tx_bps)),
    ...
    lines.push(Line::from(Span::styled("─".repeat(inner_w), theme::border_style())));
    lines.push(Line::from(vec![
        Span::styled(format!("{:>y_lbl_w$} │{:<col_w$}", "RX", " Download History"),
            Style::default().fg(theme::C_TEAL)),
        Span::styled(" │", theme::border_style()),
        Span::styled(format!("{:>y_lbl_w$} │{:<col_w$}", "TX", " Upload History"),
            Style::default().fg(theme::C_MAGENTA)),
    ]));
    ...
    for row_idx in 0..graph_h {
        let pct_lbl = if row_idx == 0 { "max".to_string() }
                      else if row_idx == graph_h / 2 { " 50%".to_string() }
                      else if row_idx + 1 == graph_h { "  0%".to_string() }
                      else { "    ".to_string() };
