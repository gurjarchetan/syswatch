...
fn render_ip_bar(f: &mut Frame, area: Rect, app: &AppState) {
    ...
    let line = Line::from(vec![
        ...
        Span::styled(state.private_ip.clone(), Style::default().fg(theme::C_TEAL).add_modifier(Modifier::BOLD)),
        Span::styled("   ◈ ", Style::default().fg(theme::C_ACCENT).add_modifier(Modifier::BOLD)),
        Span::styled("Public  ", theme::dim_style()),
        Span::styled(state.public_ip.clone(), Style::default().fg(theme::C_MAGENTA).add_modifier(Modifier::BOLD)),
    ]);
...
fn render_proc_snapshot(f: &mut Frame, area: Rect, app: &AppState) {
    ...
    lines.push(Line::from(vec![
        ...
        Span::styled(format!("  {:>5}  {:>bar_w$}", "MEM%", ""),
    ...
    for proc in state.processes.iter().take(max_rows) {
        ...
        let cpu_filled = ((proc.cpu_pct / 100.0) * bar_w as f32).round().clamp(0.0, bar_w as f32) as usize;
        let mem_filled = ((proc.mem_pct / 100.0) * bar_w as f32).round().clamp(0.0, bar_w as f32) as usize;
        let mut cpu_bar = String::with_capacity(bar_w * 3);
        for i in 0..bar_w { cpu_bar.push(if i < cpu_filled { '█' } else { '░' }); }
        let mut mem_bar = String::with_capacity(bar_w * 3);
        for i in 0..bar_w { mem_bar.push(if i < mem_filled { '█' } else { '░' }); }
...
fn render_net_summary(f: &mut Frame, area: Rect, app: &AppState) {
    ...
    let (rx_bps, tx_bps, total_rx, total_tx, rx_peak, tx_peak, rx_rows, tx_rows) = {
        ...
        (
            state.network.rx_bps, state.network.tx_bps,
            state.network.total_rx, state.network.total_tx,
            rx_peak, tx_peak, rx_rows, tx_rows,
        )
    };
    ...
    // Speed summary rows
    lines.push(Line::from(vec![
        Span::styled("▼ RX ", Style::default().fg(theme::C_TEAL).add_modifier(Modifier::BOLD)),
        Span::styled(NetStats::fmt_speed(rx_bps),
            Style::default().fg(theme::C_WHITE).add_modifier(Modifier::BOLD)),
        Span::styled(format!("  total {}", NetStats::fmt_bytes(total_rx)), theme::dim_style()),
    ]));
    lines.push(Line::from(vec![
        Span::styled("▲ TX ", Style::default().fg(theme::C_MAGENTA).add_modifier(Modifier::BOLD)),
        Span::styled(NetStats::fmt_speed(tx_bps),
            Style::default().fg(theme::C_WHITE).add_modifier(Modifier::BOLD)),
        Span::styled(format!("  total {}", NetStats::fmt_bytes(total_tx)), theme::dim_style()),
    ]));
    ...
    if each_h >= 2 {
        ...
        for (i, row_str) in rx_rows.iter().enumerate() {
            let spd = if i == 0 { shorten_speed(rx_peak) } else { "    ".to_string() };
            lines.push(Line::from(vec![
        ...
        lines.push(Line::from(Span::styled(
            ...
            Style::default().fg(theme::C_TEAL).add_modifier(Modifier::DIM),
        )));

        // TX graph
        for (i, row_str) in tx_rows.iter().enumerate() {
            let spd = if i == 0 { shorten_speed(tx_peak) } else { "    ".to_string() };
            lines.push(Line::from(vec![
                Span::styled(format!("{}|", spd), theme::dim_style()),
        ...
        lines.push(Line::from(Span::styled(
            ...
            Style::default().fg(theme::C_MAGENTA).add_modifier(Modifier::DIM),
        )));
    }
...
fn shorten_speed(bps: u64) -> String {
    if bps == 0              { "   0".to_string() }
    else if bps >= 1_073_741_824 { format!("{:>3}G", bps / 1_073_741_824) }
    else if bps >= 1_048_576    { format!("{:>3}M", bps / 1_048_576) }
    else if bps >= 1024         { format!("{:>3}K", bps / 1024) }
    else                        { format!("{:>3}B", bps) }
}
...
fn render_disk_summary(f: &mut Frame, area: Rect, app: &AppState) {
    ...
    if disks.is_empty() {
        ...
        for disk in disks {
            ...
            bar_spans.push(Span::styled(
                format!(" {:>6}", size_s), theme::dim_style(),
            ));
            bar_spans.push(Span::styled(
